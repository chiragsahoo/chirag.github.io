...
function MetricValue({
  ...
  const count = useCountUp(Number.isFinite(numeric) ? numeric : 0, visible && animate);
...
export function Snapshot() {
  ...
  return (
    <section id="snapshot" className="section snapshot" ref={ref} aria-labelledby="snapshot-title">
      <div className="container">
        <Reveal>
          <header className="section-header">
            ...
            <p className="section-subtitle">
              A concise view of leadership tenure, domain depth, and engineering focus.
            </p>
          </header>
        </Reveal>
        ...
        <div className="snapshot__grid">
          {snapshotMetrics.map((metric, i) => (
            <Reveal key={metric.description} delay={(Math.min(i + 1, 4) as 1 | 2 | 3 | 4)}>
